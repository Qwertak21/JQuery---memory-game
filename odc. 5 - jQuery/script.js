...
function generateBoard(difficulty)		//Przygotowanie nowej gry
{
	...
	{
		boardValue += '<div class="card cardStyle"></div>'; 
	...
	{
		$(".card").eq(i).on("click", function() { revealCard(i); } ); 
	}
...
{
	var opacityValue = $(".card").eq(nr).css('opacity');
	if(opacityValue > 0.1 && !lock && nr != visibleNr)
	{
		lock = true;
		var picture = "url(img/" + cards[nr] + ")";
	
		$(".card").eq(nr).css('background-image', picture);
		$(".card").eq(nr).addClass('cardA');
		$(".card").eq(nr).removeClass('cardStyle');
...
{
	$(".card").eq(nr1).css('opacity', '0.1');
	$(".card").eq(nr2).css('opacity', '0.1');
	pairsLeft--;
...
{
	play(1);
	$(".card").eq(nr1).css('background-image', 'url(img/karta.jpg)');
	$(".card").eq(nr1).addClass('cardStyle');
	$(".card").eq(nr1).removeClass('cardA');
	
	$(".card").eq(nr2).css('background-image', 'url(img/karta.jpg)');
	$(".card").eq(nr2).addClass('cardStyle');
	$(".card").eq(nr2).removeClass('cardA');
	changePlayer();
	visibleNr = 30;
	lock = false;
}
